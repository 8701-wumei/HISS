...
def main():
    ...
    parser.add_argument("--rcc_memory_capacity", type=int, default=96,
                        help="Capacity of the HISS high-risk memory bank.")  # 32 samples times 3 rounds.
    parser.add_argument("--rcc_rbf", choices=["mean", "max", "softmax", "topk_mean", "normalized_sum"], default="topk_mean",
                        help="RBF type.")
    parser.add_argument("--rcc_topk", type=int, default=3,
                        help="Top-k mean of RBF similarity.")  # k=3,5  
    parser.add_argument("--rcc_tau", type=float, default=0.3,
                        help="Softmax of RBF similarity.")    # tau=0.3
    parser.add_argument("--rcc_gamma", type=float, default=0.5,
                        help="Normalized sum of RBF similarity.")    # gamma=0.5,0.7
    parser.add_argument("--rcc_clip_quantile", type=float, default=None,
                        help="Softmax penalty clipping.")
    parser.add_argument("--rcc_lambda_base", type=float, default=1.0,
                        help="Base lambda value used in HISS score: risk - lambda * novelty.")
    parser.add_argument("--rcc_min_sigma", type=float, default=1e-6,
    ...
    parser.add_argument("--temperature", type=float, default=1.0,
                        help="Temperature for scaling prediction differences.")
    parser.add_argument("--weighted", type=bool, default=False,
                        help="Whether to weight pairs by true-score difference.")
    parser.add_argument("--top_ratio", type=float, default=None,
                        help="top-biased pair selection.")
    # GroupDRO parameters.
    ...
    if args.method == "HISS":  # epoch 1 use erm to update classifier and also update proxy
        if args.rcc_rbf == "topk_mean":
            rbf_type_name = f"{args.rcc_rbf}={args.rcc_topk}"
        elif args.rcc_rbf == "normalized_sum":
            rbf_type_name = f"{args.rcc_rbf}={args.rcc_gamma}"
        elif args.rcc_rbf == "softmax":
            rbf_type_name = f"{args.rcc_rbf}={args.rcc_tau}"
        else:
            rbf_type_name = args.rcc_rbf
        save_path = f"./{backbone}_Results/Results_norm_{args.method}_update_proxy_{args.proxy_spearman_low}_seed={args.seed}_{DATASET}_seq={args.seq_length}_{args.batch_size}_{args.learning_rate}_{args.rm_lr}_lambda={args.rcc_lambda_base}_{args.rcc_memory_capacity}_{rbf_type_name}"
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        if args.method == 'ERM':
            ...
            rcc_batch_size = batch_size if use_warmup_epoch else robust_candidate_batch_size
            for i, (src_batch, tgt_batch, seg_batch, soft_tgt_batch) in enumerate(batch_loader(rcc_batch_size, src, tgt, seg, soft_tgt)):
                start_time = time.perf_counter()
                if use_warmup_epoch:
                    loss, loss_ = train_model(args, model, optimizer, scheduler, src_batch, tgt_batch, seg_batch, soft_tgt_batch)
            ...
            for i, (src_batch, tgt_batch, seg_batch, soft_tgt_batch) in enumerate(batch_loader(rcc_batch_size, src, tgt, seg, soft_tgt)):
                start_time = time.perf_counter()
                if not hasattr(riskmodel, "_rcc_memory_rounds"):
                    memory_rounds = max(1, int(args.rcc_memory_capacity))
                    riskmodel._rcc_memory_rounds = deque(maxlen=memory_rounds)
                if use_warmup_epoch:
                    loss, loss_ = train_model(args, model, optimizer, scheduler, src_batch, tgt_batch, seg_batch, soft_tgt_batch)
                    # Update the proxy during warm-up as well.
                    src_batch = src_batch.to(args.device)
                    ...
                    sigma = compute_rbf_bandwidth(selector_embeddings, min_sigma=args.rcc_min_sigma)

                    if len(riskmodel._rcc_memory_rounds) == 0:
                        ...
                        memory_embeddings = torch.cat(list(riskmodel._rcc_memory_rounds), dim=0).to(selector_embeddings.device)
                        novelty_penalty = compute_rbf_novelty_penalty(
                            selector_embeddings,
                            memory_embeddings,
                            sigma,
                            reduction=args.rcc_rbf,
                            topk=args.rcc_topk,
                            gamma=args.rcc_gamma,
                            tau=args.rcc_tau,
                            clip_quantile=args.rcc_clip_quantile
                        )

                    lambda_value = float(args.rcc_lambda_base)
                    ...
                    riskmodel._rcc_memory_rounds.append(selected_selector_embeddings.detach().cpu())
